    ...
    def call(self, inputs):
        for i in range(len(self.utility_matrix)):
            if i == 0:
                ...
                omega1 = tf.multiply(inputs[:, -1], tf.reduce_max(self.utility_matrix[i]), name=None)
                omega2 = tf.multiply(inputs[:, -1], tf.reduce_min(self.utility_matrix[i]), name=None)
                omega = tf.expand_dims(self.nu * omega1 + (1 - self.nu) * omega2, -1)
